...
function setup() {
  ...
  // Event listeners
  uploadRadio.changed(onSourceChange);
  webcamRadio.changed(onSourceChange);
...
function getCurrentSourceImage() {
  ...
  if (activeSource === "webcam" && capture) {
    let tempGfx = createGraphics(SINGLE_WIDTH, CANVAS_HEIGHT);
    drawSourceCoverToBuffer(tempGfx, capture);
    let snap = tempGfx.get();
    tempGfx.remove();
    return snap;
  }  
  return null;
...
function onSourceChange() {
  if (uploadRadio.elt.checked) {
    activeSource = "upload";
    stopWebcam();
    needsUpdate = true;
    tilesNeedRebuild = true;
  } else if (webcamRadio.elt.checked) {
    activeSource = "webcam";
    startWebcam();
    tilesNeedRebuild = true;
  }
